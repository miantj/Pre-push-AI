...
| `ai-code-review.json` | 程序读取的配置（编辑时有中文 Schema 提示） |

也可在 Cursor **设置 → 搜 AI Code Review** 调整默认值（首次启用前）；**启用后只改 `.cursor/ai-code-review.json` 和 `.cursor/ai-code-review-prompt.md` 即可**，不必维护 `.vscode/settings.json`。
...
| `AI_CODE_REVIEW_ALLOW_ISSUES` | FAIL 仍放行（1） | 拦截 |
