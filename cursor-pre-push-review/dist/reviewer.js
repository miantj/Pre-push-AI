...
function resolveBugReviewBackend() {
...
function normalizeRemoteBranch(branch) {
    const trimmed = branch.trim();
    if (!trimmed)
        return "origin/main";
    return trimmed.startsWith("origin/") ? trimmed : `origin/${trimmed}`;
}
function getFetchBranchRef(branch) {
...
function runReview(startDir, options = {}) {
    ...
    if (withRebase && rebaseBranch) {
        const branch = (0, repo_1.getCurrentBranch)(repoRoot);
        ...
        if (branch === "main" || branch === mainName) {
            ...
            console.log(`[cursor-pre-push] 执行 git fetch origin ${fetchRef} && git rebase ${remoteBranch}`);
            try {
                const { execSync } = require("child_process");
                execSync(`git fetch origin ${fetchRef}`, { cwd: repoRoot, stdio: "inherit" });
    ...
    const verdict = (0, verdict_1.parseReviewVerdict)(combined);
    if (!allowIssues) {
        if (verdict === "FAIL") {
            (0, report_1.emitReviewFailureBlock)({
