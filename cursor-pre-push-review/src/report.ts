...
const REPORT_BASENAME = "ai-code-review-last.md";

export function reviewReportPath(repoRoot: string): string {
  return path.join(repoRoot, ".cursor", REPORT_BASENAME);
}
...
function verdictLabelZh(verdict: Verdict): string {
  if (verdict === "PASS") return "通过（PASS）";
  if (verdict === "FAIL") return "未通过（FAIL）";
  return "未能解析结论行";
}

function pushBehaviorHint(verdict: Verdict, allowIssues: boolean): string {
...
export function formatReviewReportFile(
  ...
  return [
    ...
    `- **审查结论：** ${verdictLabelZh(verdict)}`,
    `- **Push 提示：** ${pushBehaviorHint(verdict, allowIssues)}`,
    "",
    "## 1. 先看这里：怎么读",
    "",
