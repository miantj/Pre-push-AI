...
function buildFileStructure(): string[] {
  return [
    ...
    "| `ai-code-review/env` | Provider 模式下 Git hook 读取的 API Key |",
...
function buildUsageScenarios(): string[] {
  return [
    ...
    "3. 若 hook 里已有自定义脚本，扩展不会静默覆盖，需手动 **启用审查** 确认安装",
    "",
    "### 5.4 自定义审查 Prompt",
    "",
...
function buildHookGuide(hooks: string[]): string[] {
  ...
  lines.push(
    ...
    "",
    "- **hook.sh 不存在**（如新 clone 未运行扩展）：仅警告并跳过，不阻断 push",
...
function buildAgentProviderGuide(config: AiCodeReviewConfig): string[] {
  const lines = [
    ...
    "",
    `设置 API Key 命令会 **同时写入 SecretStorage 和 env 文件**。手动编辑 env 时，含特殊字符的 key 请用单引号包裹，例如 \`AI_CODE_REVIEW_API_KEY='your-key'\`。`,
    "",
  ];
...
function buildFaq(): string[] {
  return [
    ...
    "",
    "**Q: Provider 请求失败？**",
    "→ 检查 `provider.type` / `model` / `baseUrl` / `path` 及 API Key 是否有效。",
    "",
